...
pressure_level, address: 140; Uint16, Pressure in feeding pipe driven by pump, normal level is 90-100, above is overpressure
...
chlorine_lvl, address: 120; Uint16, adjustable value for chlorine intake level, does not affect rest of procees, for addtional attacking scenario (like Oldsmar)
"""
# --------------------------------------------------------------------------- #
...
#------------------------------------------------
	...
	slave_id = 0x00 #Slave ID is set to 0
	...
	pressure_level = context[slave_id].getValues(4, 140, count=1) 
	pipe_strength = 500 # How much pressure the pipe will hold before rupture
	...
	tank_HH = 65500  #tank maximum capacity (Overflow level)
	...
	tank_Lo = context[slave_id].getValues(3, 110, count=1)  #Motor start Setpoint capacity (Hi alarm)
	chlorine_lvl = context[slave_id].getValues(3, 120, count=1)  #Chlorine level
...
#-------------------------
	if mot_run[0]==True: #-----checking running state and running process if True
		...
		if valve_state[0] == True: 
			pressure_level[0] = 90+random.randint(0, 10)  # simulate random pressure load on open valve
        
     #tripping motor on valve state      
		if valve_state[0] == False: 
			#ifdef Debug
			log.warning('Valve closed on running pump! Pressure increasing')
			#endif
			if pressure_level[0] > pipe_strength:
				log.warning('Overpressure! Pipe exploded, pump and motor is broken')			
				log.warning('!!!!!!!!!!!!!!!!!!!!!!!!!!!Simulation stopped!!!!!!!!!!!!!!!!!!!!!!!!!!!')
				raise SystemExit				
			else: pressure_level[0] = pressure_level[0]+1


     #tripping on full condition(HiHi trip) 
		if tank_level[0]>tank_HH: 
			#ifdef Debug
			log.warning('Tank overflow, water flooded control panel!') 
			log.warning('!!!!!!!!!!!!!!!!!!!!!!!!!!!Simulation stopped!!!!!!!!!!!!!!!!!!!!!!!!!!!')
			raise SystemExit

     #Warning on chlorine level increase(HiHi trip) 
		if chlorine_lvl[0]>700: 
			#ifdef Debug
			log.warning('!!!!!!!!!!!!!!Chlorine level increased, vater is poisonous!!!!!!!!!!!!!!!!!!!!!!') 
   ...
   #---------------------------------
	...
	context[slave_id].setValues(4, 140, pressure_level)
   #Values output to monitor process state, if not required change to log.debug along with setting logging level to info
	log.info(f"Process values:Motor running: {mot_run[0]} |Auto mode: {mot_auto[0]} |Motor load: {mot_load[0]} |Pressure: {pressure_level[0]} |Flow: {flowm_flow[0]} |Tank level: {tank_level[0]} |Tank drain: {tank_drain[0]} |Start signal: {mot_start[0]} |Stop signal: {mot_stop[0]} |Chlorine: {chlorine_lvl[0]}") 
