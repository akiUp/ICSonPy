...
#------------------------------------------------
	...
	tank_HH = 6550  #tank maximum capacity (Overflow level)
...
#-------------------------
   ...
   #Values output to monitor process state, if not required change to log.debug along with setting logging level to info
	log.info(f"Process values:Motor running: {mot_run[0]} |Auto mode: {mot_auto[0]} |Motor load: {mot_load[0]} |Pressure: {pressure_level[0]} |Flow: {flowm_flow[0]} |Tank level: {tank_level[0]} |Tank drain: {tank_drain[0]} |Start signal: {mot_start[0]} |Stop signal: {mot_stop[0]}") 
